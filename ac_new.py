...
def import_images(datum):
  datum = datum.split(' ')
  if(os.path.exists('/home/shivam/desktop/im2latex/images_processed/'+datum[0])):
  	img = np.array(Image.open('/home/maneeshita/Downloads/im2latex-master/data/images_processed2/'+datum[0]).convert('L'))
...
for i in range(0,len(val)):
	...
	if(os.path.exists('/home/ec2-user/images_(160_500)/'+str(y[0]))):
		#print('Shivam pagal hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh')
...
#print('heeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee')
...
def init_cnn(inp):
  ...
  W_conv2 = weight_variable("conv2", [3,3,128,128]) #change
...
cnn_c=tf.reshape(next_layer,[batch_size,1700,emb_size])
next_layer=tf.reshape(next_layer,[batch_size,1700,emb_size])
...
time=0
final_out=tf.zeros([batch_size],dtype=tf.int64)
logits_for_softmax=tf.zeros([batch_size,vocab_size],dtype=tf.float32)
loss=0
...
while(time < max_decode_length-1):
	...
	logits_for_softmax=tf.concat([logits_for_softmax,logits],0)
	sample_ids = tf.argmax(logits, axis=-1)
	next_inputs = tf.nn.embedding_lookup(embed, sample_ids)
	
	final_out=tf.concat([final_out,sample_ids],0)
...
with sess.as_default():
	for epoch in range(0,1):
		...
		for i in range(0,2):
			...
			print("Predictions")
			print(final_out.eval({inp:images,label:labels}))
			print("labels")
			print(labels)
		...
		batch_loss=open("batch_loss.txt","a")
		batch_loss.write("epoch "+str(epoch)+str(l/len(input_train)+"\n"))
		batch_loss.close()
		print()
		print()
		print()
