...
train = open('/home/maneeshita/Downloads/im2latex-master/data/train_filter2.lst').read().split('\n')[:-1]
val = open('/home/maneeshita/Downloads/im2latex-master/data/validate_filter.lst').read().split('\n')[:-1]
test = open('/home/maneeshita/Downloads/im2latex-master/data/test_filter3.lst').read().split('\n')[:-1]
train_list=[]
test_list=[]
val_list=[]
...
for i in range(0,len(val)):
  ...
  y=val[i].split(' ')
  if(os.path.exists("/home/maneeshita/Downloads/im2latex-master/data/images_processed2/"+str(y[0]))):
    img = np.array(Image.open("/home/maneeshita/Downloads/im2latex-master/data/images_processed2/"+str(y[0])).convert('L'))
    #train(img, formulas[ int(train[1]) ])
...
for i in range(0,len(test)):
  z=test[i].split(' ')
  if(os.path.exists("/home/maneeshita/Downloads/im2latex-master/data/images_processed2/"+str(z[0]))):
    img = np.array(Image.open("/home/maneeshita/Downloads/im2latex-master/data/images_processed2/"+str(z[0])).convert('L'))
    #train(img, formulas[ int(train[1]) ])
...
def conv2d_weight_norm(inputs,name, in_dim,out_dim,dropout=1.0, padding="SAME"):
  with tf.variable_scope(name):
    V = tf.get_variable('V'+name, shape=[1,3, in_dim, out_dim], dtype=tf.float32, initializer=tf.random_normal_initializer(mean=0, stddev=tf.sqrt(4.0*1.0/(3*in_dim))), trainable=True)
...
with sess.as_default():
	...
	for ep in range(0,epochs):
      ...
      #print(sess.run(sz.eval({inp:images,label:labels})))
			...
			pred=np.split(output.eval({inp:images,label:labels,pad:[max_true],is_training:[0]}),batch_size)
			print(pred)
			print(labels)
			train_labels=open("epoch-"+str(ep)+"train_labels.txt","a")
			...
			if(i%2750==0 and i >0 ):                                              ########commented
				id = "/home/ec2-user/epoch2/saved_models/model-epoch"+str(epoch)+" "+str(i)
				os.mkdir(id)
			...
			if(i%zx==0 and i>0):
				...
				val=open("val.txt","a")
				val.write("epoch-"+str(ep)+"-"+str(i)+" "+str(l_val/20)+"\n")
				val.close()
		...
		#id = str(epoch)
		#id = "/home/ec2-user/ole2/saved_models/model-epoch"+str(ep) ########commented
		#os.mkdir(id)
		#save_path = saver.save(sess, id+'/model' )
		epoch_loss.write("epoch "+str(ep)+"-"+str((l/len(input_train)))+"\n")
		epoch_loss.close()
		...
		for i in range(0,len(input_test)):
			...
			pred=sess.run((final_out),({inp:images,label:labels,pad:[max_true_t],is_training:[0]}))
			#print(pred)
			pred=np.split(pred,batch_size)
			#print(pred)
			...
			test_labels=open("epoch-"+str(ep)+"test_labels.txt","a")
			for j in range(0,batch_size):
		...
		test_acc.write("test-epoch-"+str(ep)+"-"+str(acc)+"\n")
		test_acc.close()
	train_writer.close()
